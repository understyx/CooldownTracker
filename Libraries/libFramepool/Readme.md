...
**LibFramePool-1.0** is a lightweight World of Warcraft library designed to manage the recycling of frames. By pooling frames instead of creating new ones and discarding old ones, you significantly reduce memory churn and Garbage Collection usage, which is critical for performance on the 3.3.5a client.
...
| :--- | :--- | :--- |
| **`onAcquire`** | `function` | A custom function called immediately **after** a frame is retrieved from the pool but before it is returned to you. Signature: `func(frame)`. Useful for re-initializing state. |
| **`resetter`** | `function` | A custom function called immediately **before** the frame is stored (released). Signature: `func(frame)`. Useful for cleaning up custom data. |
...
| **`resetParent`** | `Frame` | If set, the frame is re-parented to this object upon release. **If omitted, the frame is detached (`SetParent(nil)`) to prevent UI taint.** |

---

## Usage Examples

### 1. Basic Frame Pooling (3.3.5a Compatible)
...
local function CreateMyButton(parent)
    ...
    tex:SetAllPoints()
    tex:SetTexture(1, 0, 0, 0.5) -- 3.3.5a syntax (r, g, b, a)
    btn.tex = tex
...
When a frame is released, the following standard resets always occur automatically **before** the `resetter` function is called:

* `:Hide()`
* `:ClearAllPoints()`
* `:SetParent(nil)` (Unless `resetParent` option is used)
* `:SetAlpha(1)`
* `:SetScale(1)`
